...
def main(request):
    ...
    for i in range(3):
        ...
        pdf_url = generate_and_test_menu_url(target_date)
    ...
    if pdf_content:
        try:
            ...
            if now_jst.hour == 7 and now_jst.min == 0:
                message_text = (
            ...
            elif now_jst.hour == 12 and now_jst.min == 0:
                message_text = (
                ...
                message_text = (
                    f"【TEST寮食メニュー ({today.strftime('%-m/%-d')})】\n\n"
                    f"■ 朝食\n{menu_asa}\n\n"
                    f"■ 昼食\n{menu_hiru}\n\n"
                    f"■ 夕食\n{menu_yoru}"
                )
